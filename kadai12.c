...
void update_ranking(struct ranking_data_t ranking[], double score,char pre_name[MAX_NAME]){
	int rank = get_rank(ranking,score);
	if(rank == -1) return;
  int new_rank = rank - 1;
...
int main (void){
  ...
  ranking[0].score=1.2;//rankの初期値設定//ok
	for(int i=1;i<RANKING_SIZE;i++){
		ranking[i].score=ranking[i-1].score+0.5;
	}

	for(int i=0;i<RANKING_SIZE;i++){
		int j=0;
		for(j=0;j<5;j++){
			int make_random_name= (random()%(26)+97);
